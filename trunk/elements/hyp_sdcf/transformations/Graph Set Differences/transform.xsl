...
<xsl:variable name="GraphTitle">SD-CF graph - set differences</xsl:variable>
		...
		<paragraph>
			...
			<xsl:for-each select="key('key_ti_attribute',@attributes)">
				...
				<xsl:element name="chart">
					...
					<xsl:attribute name="title">first set</xsl:attribute>				
				...
				<xsl:element name="chart">
					...
					<xsl:attribute name="title">second set</xsl:attribute>				
