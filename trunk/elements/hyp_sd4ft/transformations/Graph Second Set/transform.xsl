...
<xsl:variable name="GraphTitle">SD-4ft Graph - second set</xsl:variable>
...
<xsl:variable name="GraphHeight">0,75</xsl:variable>
	...
	<xsl:variable name="id_base" select="@id" />
		...
		<paragraph>
			<graph title="{$GraphTitle}" showlegend="{$LegendShow}" charttype="{$GraphType}" height="{$GraphHeight}" width="{$GraphWidth}" id="{$id_base}_4ftgraph2">
