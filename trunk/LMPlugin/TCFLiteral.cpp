// TCFLiteral.cpp : implementation file
//
/*
...
IMPLEMENT_DYNAMIC(TCFLiteral, CRecordset)

TCFLiteral::TCFLiteral(CDatabase* pdb)
	: CRecordset(pdb)
{
	//{{AFX_FIELD_INIT(TCFLiteral)
	m_CFLiteralDID = 0;
	m_CFCedentDID = 0;
	m_QuantityID = 0;
	m_Ord = 0;
...
CString TCFLiteral::GetDefaultSQL()
{
	return _T("[tdCFLiteralD],[tmAttribute],[tmQuantity]");
}

void TCFLiteral::DoFieldExchange(CFieldExchange* pFX)
{
	//{{AFX_FIELD_MAP(TCFLiteral)
	pFX->SetFieldType(CFieldExchange::outputColumn);
	RFX_Long(pFX, _T("[CFLiteralDID]"), m_CFLiteralDID);
	RFX_Long(pFX, _T("[CFCedentDID]"), m_CFCedentDID);
	RFX_Long(pFX, _T("[tdCFLiteralD].[QuantityID]"), m_QuantityID);
	RFX_Long(pFX, _T("[Ord]"), m_Ord);
	RFX_Text(pFX, _T("[tdCFLiteralD].[Notice]"), m_Notice);
...
/////////////////////////////////////////////////////////////////////////////
// TCFLiteral diagnostics

#ifdef _DEBUG
void TCFLiteral::AssertValid() const
{
	CRecordset::AssertValid();
}

void TCFLiteral::Dump(CDumpContext& dc) const
{
	CRecordset::Dump(dc);
}
#endif //_DEBUG
