// Category_Recordset.cpp : implementation file
//
...
IMPLEMENT_DYNAMIC(Category_Recordset, CRecordset)

Category_Recordset::Category_Recordset(CDatabase* pdb)
	: CRecordset(pdb)
{
	//{{AFX_FIELD_INIT(Category_Recordset)
//	m_AttributeID = 0;
	m_Attr_Name = "";
//	m_MatrixID = 0;
...
CString Category_Recordset::GetDefaultSQL()
{
	return _T("[tmAttribute],[tmCategory],[tmMatrix],[tmQuantity],[tsCategorySubType]");
}

void Category_Recordset::DoFieldExchange(CFieldExchange* pFX)
{
	//{{AFX_FIELD_MAP(Category_Recordset)
	pFX->SetFieldType(CFieldExchange::outputColumn);
//	RFX_Long(pFX, _T("[tmAttribute].[AttributeID]"), m_AttributeID);
	RFX_Text(pFX, _T("[tmAttribute.Name]"), m_Attr_Name);
...
//	RFX_Long(pFX, _T("[tmAttribute].[wSavedCountUsed]"), m_wSavedCountUsed);
	...
	RFX_Text(pFX, _T("[tmCategory.Name]"), m_Ctgr_Name);
...
//	RFX_Long(pFX, _T("[tmMatrix].[MatrixID]"), m_MatrixID2);
	RFX_Text(pFX, _T("[tmMatrix.Name]"), m_Matrix_Name);
...
//	RFX_Long(pFX, _T("[tsCategorySubType].[CategorySubTypeID]"), m_CategorySubTypeID2);
	RFX_Text(pFX, _T("[tsCategorySubType.Name]"), m_Ctgr_tp_Name);
//	RFX_Text(pFX, _T("[tsCategorySubType].[ShortName]"), m_ShortName2);
//	RFX_Long(pFX, _T("[tsCategorySubType].[Ord]"), m_Ord2);
//	RFX_Text(pFX, _T("[tsCategorySubType].[Notice]"), m_Notice5);
	//}}AFX_FIELD_MAP
}

/////////////////////////////////////////////////////////////////////////////
// Category_Recordset diagnostics

#ifdef _DEBUG
void Category_Recordset::AssertValid() const
{
	CRecordset::AssertValid();
}

void Category_Recordset::Dump(CDumpContext& dc) const
{
	CRecordset::Dump(dc);
}
#endif //_DEBUG
